...
@dataclass(frozen=True, slots=True)
class CampaignProjectionErrorNamedAssessmentCollectionVerdictCollectionVerdict:
    """Immutable verdict over ordered named assessment-collection verdicts."""

    overall_passed: bool
    passing_verdict_names: tuple[str, ...]
    failing_verdict_names: tuple[str, ...]
    undefined_verdict_names: tuple[str, ...]


@dataclass(frozen=True, slots=True)
class CampaignProjectionErrorNamedAssessmentCollectionAggregateVerdict:
    """One explicitly named assessment-collection aggregate verdict."""
...
def campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_collection_verdict(
    entries,
):
    """Classify named assessment-collection verdicts by stored state."""
    entries = _validated_projection_error_named_assessment_collection_verdicts(
        entries
    )
    for entry in entries:
    ...
    undefined = []
    for entry in entries:
        if entry.verdict.undefined_collection_names:
            undefined.append(entry.name)
        elif entry.verdict.overall_passed:
            passing.append(entry.name)
        else:
            failing.append(entry.name)
    return CampaignProjectionErrorNamedAssessmentCollectionVerdictCollectionVerdict(
        overall_passed=bool(entries) and not failing and not undefined,
        passing_verdict_names=tuple(passing),
        failing_verdict_names=tuple(failing),
        undefined_verdict_names=tuple(undefined),
    )


def campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_collection_verdict_record(
    verdict,
):
    """Convert one named assessment-collection aggregate verdict to a record."""
    _validated_projection_error_named_assessment_collection_aggregate_verdict(
        verdict
    )
    return {
        "overall_passed": verdict.overall_passed,
        "passing_verdict_names": list(verdict.passing_verdict_names),
        "failing_verdict_names": list(verdict.failing_verdict_names),
        "undefined_verdict_names": list(verdict.undefined_verdict_names),
    }


def campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_collection_verdict_records(
    entries,
):
    """Convert ordered named assessment-collection aggregate verdicts."""
    entries = _validated_projection_error_named_assessment_collection_aggregate_verdicts(
        entries
    )
    for entry in entries:
        _validated_projection_error_named_assessment_collection_aggregate_verdict(
...
def _validated_projection_error_named_assessment_collection_aggregate_verdicts(
    entries,
):
    try:
    ...
    for index, entry in enumerate(entries):
        ...
        if entry.name in names:
            raise ValueError(
                f"duplicate assessment collection aggregate verdict name {entry.name!r}"
            )
...
def _validated_projection_error_named_assessment_collection_aggregate_verdict(
    verdict,
):
    if not isinstance(
        verdict,
    ...
    expected_pass = bool(verdict.passing_verdict_names) and not (
        verdict.failing_verdict_names or verdict.undefined_verdict_names
    )
