...
from flightlab.analysis import (
    ...
    CampaignProjectionErrorNamedAssessmentCollectionAggregateVerdict,
...
def test_named_assessment_collection_verdict_overview_one_passing_collection():
    overview = (
        campaign_projection_error_comparison_envelope_assessment_collection_verdict_overview(
            (_named_assessment_collection_report("passing"),)
        )
    )
    assert overview == [{"name": "passing", "overall_passed": True}]
    assert tuple(overview[0]) == ("name", "overall_passed")


def test_named_assessment_collection_verdict_overview_preserves_mixed_order():
    entries = (
        _named_assessment_collection_report("passing"),
        _named_assessment_collection_report(
            "failing",
            (_named_assessment_report("failed", ("fail", *("pass",) * 6)),),
        ),
        _named_assessment_collection_report(
            "undefined",
            (_named_assessment_report("unknown", ("undefined", *("pass",) * 6)),),
        ),
    )
    overview = (
        campaign_projection_error_comparison_envelope_assessment_collection_verdict_overview(
    ...
    assert overview == [
        {"name": "passing", "overall_passed": True},
        {"name": "failing", "overall_passed": False},
        {"name": "undefined", "overall_passed": False},
    ]


def test_named_assessment_collection_verdict_overview_rejects_names_and_members():
    report = _named_assessment_collection_report().report
    with pytest.raises(ValueError, match="name must be non-empty"):
        campaign_projection_error_comparison_envelope_assessment_collection_verdict_overview(
            (CampaignProjectionErrorNamedAssessmentCollectionReport(" ", report),)
        )
    same = CampaignProjectionErrorNamedAssessmentCollectionReport("same", report)
    with pytest.raises(ValueError, match="duplicate assessment collection report name"):
        campaign_projection_error_comparison_envelope_assessment_collection_verdict_overview(
            (same, same)
        )
...
def test_named_assessment_collection_verdict_overview_rejects_inconsistent_report():
    report = _named_assessment_collection_report().report
    malformed = CampaignProjectionErrorComparisonEnvelopeAssessmentCollectionReport(
        report.named_reports,
        CampaignProjectionErrorComparisonEnvelopeAssessmentCollectionVerdict(
...
def test_named_assessment_collection_verdict_overview_empty_generator_and_detached():
    assert (
        campaign_projection_error_comparison_envelope_assessment_collection_verdict_overview(
            ()
        )
...
def test_named_assessment_collection_verdict_record_preserves_stored_fields_and_order():
    ...
    assert (
        ...
        == {
            "overall_passed": False,
            "passing_collection_names": ["passing-b", "passing-a"],
            "failing_collection_names": ["failing-b", "failing-a"],
            "undefined_collection_names": ["undefined-b", "undefined-a"],
        }
    )


def test_named_assessment_collection_verdict_record_empty_schema_is_explicit():
    verdict = CampaignProjectionErrorNamedAssessmentCollectionReportVerdict(
        False, (), (), ()
    )
    assert (
...
def test_named_assessment_collection_verdict_record_is_detached_and_deterministic():
    verdict = CampaignProjectionErrorNamedAssessmentCollectionReportVerdict(
        True, ("first", "second"), (), ()
    )
    first = campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_record(
        verdict
    )
    repeated = (
        campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_record(
    ...
    assert first == repeated
    assert first is not repeated
    assert first["passing_collection_names"] is not repeated["passing_collection_names"]
    first["passing_collection_names"].append("mutated")
    first["overall_passed"] = False
    assert verdict.passing_collection_names == ("first", "second")
    assert (
        campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_record(
            verdict
        )
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            ...
            TypeError,
            "passing_collection_names must be a tuple",
        ),
        (
            CampaignProjectionErrorNamedAssessmentCollectionReportVerdict(
                False, (" ",), ("failing",), ()
...
def test_named_assessment_collection_verdict_record_rejects_invalid_verdicts(
    verdict, error, message
):
...
def test_named_assessment_collection_verdict_records_preserve_outer_and_inner_order():
    entries = (
        CampaignProjectionErrorNamedAssessmentCollectionVerdict(
            "second",
            CampaignProjectionErrorNamedAssessmentCollectionReportVerdict(
                False, ("pass-b", "pass-a"), ("fail-b", "fail-a"), ()
            ),
        ),
        CampaignProjectionErrorNamedAssessmentCollectionVerdict(
            "first",
            CampaignProjectionErrorNamedAssessmentCollectionReportVerdict(
                False, (), (), ("undefined-b", "undefined-a")
            ),
        ),
    )
    records = campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_records(
        entries
    )
    assert [record["name"] for record in records] == ["second", "first"]
    assert records[0]["verdict"]["passing_collection_names"] == [
        "pass-b",
        "pass-a",
    ]
    assert records[0]["verdict"]["failing_collection_names"] == [
        "fail-b",
        "fail-a",
    ]
    assert records[1]["verdict"]["undefined_collection_names"] == [
        "undefined-b",
        "undefined-a",
    ]


def test_named_assessment_collection_verdict_records_delegate_once_per_entry(
    monkeypatch,
):
    calls = []
    ...
    entries = tuple(
        CampaignProjectionErrorNamedAssessmentCollectionVerdict(name, verdict)
        for name in ("a", "b")
    )
    records = campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_records(
        entries
    )
    assert calls == [verdict, verdict]
    assert records == [
        {"name": "a", "verdict": {"converted": 1}},
        {"name": "b", "verdict": {"converted": 2}},
    ]


def test_named_assessment_collection_verdict_records_prevalidate_all_entries(
    monkeypatch,
):
    calls = []
    monkeypatch.setattr(
        analysis,
        "campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_record",
        lambda verdict: calls.append(verdict),
    )
    valid_verdict = CampaignProjectionErrorNamedAssessmentCollectionReportVerdict(
        True, ("passing",), (), ()
    )
    malformed_verdict = CampaignProjectionErrorNamedAssessmentCollectionReportVerdict(
        True, ("passing",), ("failing",), ()
    )
    with pytest.raises(ValueError, match="overall pass state is inconsistent"):
        campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_records(
            (
                CampaignProjectionErrorNamedAssessmentCollectionVerdict(
                    "valid", valid_verdict
...
def test_named_assessment_collection_verdict_records_reject_invalid_entries():
    verdict = CampaignProjectionErrorNamedAssessmentCollectionReportVerdict(
        True, ("passing",), (), ()
    )
    entry = CampaignProjectionErrorNamedAssessmentCollectionVerdict("same", verdict)
    with pytest.raises(ValueError, match="duplicate assessment collection verdict"):
        campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_records(
            (entry, entry)
        )
    with pytest.raises(ValueError, match="name must be non-empty"):
        campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_records(
            (CampaignProjectionErrorNamedAssessmentCollectionVerdict(" ", verdict),)
        )
    with pytest.raises(TypeError, match="NamedAssessmentCollectionVerdict"):
        campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_records(
            (object(),)
        )
    with pytest.raises(TypeError, match=r"entries\[0\].verdict"):
        campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_records(
            (CampaignProjectionErrorNamedAssessmentCollectionVerdict("bad", object()),)
        )


def test_named_assessment_collection_verdict_records_empty_generator_and_detached():
    assert (
        campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_records(
            ()
        )
    ...
    entries = [CampaignProjectionErrorNamedAssessmentCollectionVerdict("named", verdict)]
    first = campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_records(
        entry for entry in entries
    )
    repeated = campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_records(
        entries
    )
    assert first == repeated
    json.dumps(first, allow_nan=False)
    first[0]["name"] = "changed"
    first[0]["verdict"]["passing_collection_names"].append("changed")
    entries.clear()
    assert repeated == [
        {
            "name": "named",
            "verdict": {
                "overall_passed": True,
                "passing_collection_names": ["first", "second"],
                "failing_collection_names": [],
                "undefined_collection_names": [],
            },
        }
...
def test_named_assessment_collection_verdict_overview_exact_schema_and_order():
    entries = (
        CampaignProjectionErrorNamedAssessmentCollectionVerdict(
            "second",
            CampaignProjectionErrorNamedAssessmentCollectionReportVerdict(
    ...
    assert campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_overview(
        entries
    ) == [
        {"name": "second", "overall_passed": False},
        {"name": "first", "overall_passed": True},
    ]


def test_named_assessment_collection_verdict_overview_reuses_stored_pass_state(
    monkeypatch,
):
    monkeypatch.setattr(
        analysis,
        "campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_record",
        lambda verdict: pytest.fail("record conversion must not be used"),
    )
    entry = CampaignProjectionErrorNamedAssessmentCollectionVerdict(
        "undefined",
        CampaignProjectionErrorNamedAssessmentCollectionReportVerdict(
            False, (), (), ("stored-undefined",)
        ),
    )
    assert campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_overview(
        (entry,)
...
def test_named_assessment_collection_verdict_overview_prevalidates_all_input():
    valid = CampaignProjectionErrorNamedAssessmentCollectionVerdict(
        "valid",
        CampaignProjectionErrorNamedAssessmentCollectionReportVerdict(
            True, ("passed",), (), ()
        ),
    )
    invalid = CampaignProjectionErrorNamedAssessmentCollectionVerdict(
        "invalid",
        CampaignProjectionErrorNamedAssessmentCollectionReportVerdict(
            True, ("passed",), ("failed",), ()
        ),
...
def test_named_stored_assessment_collection_verdict_overview_empty_and_detached():
    assert (
        campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_overview(
            ()
        )
    ...
    entries = [CampaignProjectionErrorNamedAssessmentCollectionVerdict("named", verdict)]
    first = campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_overview(
        entry for entry in entries
    )
    repeated = campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_overview(
        entries
    )
    assert first == repeated == [{"name": "named", "overall_passed": True}]
    assert first is not repeated
    first[0]["name"] = "changed"
    first[0]["overall_passed"] = False
    entries.clear()
    assert repeated == [{"name": "named", "overall_passed": True}]
    assert verdict.overall_passed is True
    json.dumps(repeated, allow_nan=False)


def test_named_assessment_collection_verdict_collection_verdict_all_passing():
    stored = CampaignProjectionErrorNamedAssessmentCollectionReportVerdict(
        True, ("passing",), (), ()
    )
    verdict = campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_collection_verdict(
        (
            CampaignProjectionErrorNamedAssessmentCollectionVerdict("first", stored),
            CampaignProjectionErrorNamedAssessmentCollectionVerdict("second", stored),
        )
    )
    assert verdict == (
        CampaignProjectionErrorNamedAssessmentCollectionVerdictCollectionVerdict(
...
def test_named_assessment_collection_verdict_collection_verdict_classifies_failures():
    passing = CampaignProjectionErrorNamedAssessmentCollectionReportVerdict(
        True, ("passing",), (), ()
    )
    failing = CampaignProjectionErrorNamedAssessmentCollectionReportVerdict(
        False, ("passing",), ("failing",), ()
    )
    verdict = campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_collection_verdict(
        (
            CampaignProjectionErrorNamedAssessmentCollectionVerdict("pass", passing),
            CampaignProjectionErrorNamedAssessmentCollectionVerdict("fail-a", failing),
            CampaignProjectionErrorNamedAssessmentCollectionVerdict("fail-b", failing),
        )
    )
    assert verdict.overall_passed is False
    assert verdict.passing_verdict_names == ("pass",)
    assert verdict.failing_verdict_names == ("fail-a", "fail-b")
    assert verdict.undefined_verdict_names == ()


def test_named_assessment_collection_verdict_collection_verdict_undefined_precedence():
    undefined = CampaignProjectionErrorNamedAssessmentCollectionReportVerdict(
        False, (), ("failed",), ("undefined",)
    )
    verdict = campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_collection_verdict(
        (CampaignProjectionErrorNamedAssessmentCollectionVerdict("mixed", undefined),)
    )
    assert verdict.passing_verdict_names == ()
    assert verdict.failing_verdict_names == ()
    assert verdict.undefined_verdict_names == ("mixed",)


def test_named_assessment_collection_verdict_collection_verdict_category_order():
    states = {
        "pass": CampaignProjectionErrorNamedAssessmentCollectionReportVerdict(
            True, ("passing",), (), ()
        ),
        "fail": CampaignProjectionErrorNamedAssessmentCollectionReportVerdict(
            False, (), ("failing",), ()
        ),
        "undefined": CampaignProjectionErrorNamedAssessmentCollectionReportVerdict(
            False, (), (), ("undefined",)
    ...
    entries = tuple(
        CampaignProjectionErrorNamedAssessmentCollectionVerdict(name, states[state])
        for name, state in (
    ...
    assert verdict.passing_verdict_names == ("pass-b", "pass-a")
    assert verdict.failing_verdict_names == ("fail-b", "fail-a")
    assert verdict.undefined_verdict_names == ("undefined-b", "undefined-a")


def test_named_assessment_collection_verdict_collection_verdict_validates_all_input():
    valid = CampaignProjectionErrorNamedAssessmentCollectionVerdict(
        "valid",
        CampaignProjectionErrorNamedAssessmentCollectionReportVerdict(
            True, ("passing",), (), ()
...
def test_named_assessment_collection_verdict_collection_verdict_empty_deterministic():
    empty = CampaignProjectionErrorNamedAssessmentCollectionVerdictCollectionVerdict(
        False, (), (), ()
    )
    assert campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_collection_verdict(
        ()
    ) == empty
    source = [
        CampaignProjectionErrorNamedAssessmentCollectionVerdict(
    ...
    first = campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_collection_verdict(
        entry for entry in source
    )
    repeated = campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_collection_verdict(
        source
    )
    assert first == repeated
    assert first is not repeated
...
def test_named_assessment_collection_aggregate_verdict_records_schema_and_order():
    entries = (
        CampaignProjectionErrorNamedAssessmentCollectionAggregateVerdict(
            "second",
            CampaignProjectionErrorNamedAssessmentCollectionVerdictCollectionVerdict(
    ...
    assert campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_collection_verdict_records(
        entries
    ) == [
        {
            "name": "second",
...
def test_named_assessment_collection_aggregate_verdict_records_delegate_once(
    monkeypatch,
):
    calls = []
    ...
    entries = tuple(
        CampaignProjectionErrorNamedAssessmentCollectionAggregateVerdict(name, verdict)
        for name in ("a", "b")
    )
    records = campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_collection_verdict_records(
        entries
    )
    assert calls == [verdict, verdict]
    assert records == [
...
def test_named_assessment_collection_aggregate_verdict_records_prevalidate_all(
    monkeypatch,
):
    calls = []
    monkeypatch.setattr(
    ...
    valid = CampaignProjectionErrorNamedAssessmentCollectionVerdictCollectionVerdict(
        True, ("passing",), (), ()
    )
    invalid = CampaignProjectionErrorNamedAssessmentCollectionVerdictCollectionVerdict(
        True, ("passing",), ("failing",), ()
    )
    with pytest.raises(ValueError, match="overall pass state is inconsistent"):
        campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_collection_verdict_records(
...
def test_named_assessment_collection_aggregate_verdict_records_validate_entries():
    verdict = CampaignProjectionErrorNamedAssessmentCollectionVerdictCollectionVerdict(
        True, ("passing",), (), ()
    )
    entry = CampaignProjectionErrorNamedAssessmentCollectionAggregateVerdict(
        "same", verdict
    )
    with pytest.raises(ValueError, match="duplicate assessment collection aggregate"):
        campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_collection_verdict_records(
            (entry, entry)
        )
    with pytest.raises(ValueError, match="name must be non-empty"):
    ...
    with pytest.raises(TypeError, match="NamedAssessmentCollectionAggregateVerdict"):
        campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_collection_verdict_records(
            (object(),)
        )
    with pytest.raises(TypeError, match=r"entries\[0\].verdict"):
...
def test_named_assessment_collection_aggregate_verdict_records_empty_detached():
    assert campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_collection_verdict_records(
        ()
    ) == []
    verdict = CampaignProjectionErrorNamedAssessmentCollectionVerdictCollectionVerdict(
    ...
    first = campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_collection_verdict_records(
        entry for entry in entries
    )
    repeated = campaign_projection_error_comparison_envelope_named_assessment_collection_verdict_collection_verdict_records(
        entries
    )
    assert first == repeated
    json.dumps(first, allow_nan=False)
    first[0]["name"] = "changed"
    first[0]["verdict"]["passing_verdict_names"].append("changed")
    entries.clear()
    assert repeated[0]["name"] == "named"
    assert repeated[0]["verdict"]["passing_verdict_names"] == ["first", "second"]
    assert verdict.passing_verdict_names == ("first", "second")
